...
csv_out = "C:\\Users\\whf80\\Desktop\\Car-Dataset\\CAN-MIRGU-main\\enhanced_CAN-MIRGU-n-gram.csv"
win_size_dt = 50    # Δt
win_size_id = 100   # ID 
...
df['payload_str'] = bytes_df.apply(
    lambda row: ' '.join(f'{b:02X}' for b in row), axis=1
)

# Extract 1-gram, 2-gram, 3-gram by CountVectorizer
vect = CountVectorizer(
    token_pattern=r'[^ ]+',
    analyzer='word',
    ngram_range=(1, 2)
)
X_ng = vect.fit_transform(df['payload_str'])

ngram_df = pd.DataFrame.sparse.from_spmatrix(
    X_ng,
    index=df.index,
    columns=[f'ng_{g.replace(" ","_")}' for g in vect.get_feature_names_out()]
)
df = pd.concat([df, ngram_df], axis=1)

df.drop(columns=['payload_str','id_code'], errors='ignore', inplace=True)

df.to_csv(csv_out, index=False)
print(f"Enhanced data saved to {csv_out}")
