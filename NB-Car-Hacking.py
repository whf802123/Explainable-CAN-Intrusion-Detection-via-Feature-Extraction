...
# Feature normalization for GaussianNB
scaler = StandardScaler()
X_train_sd = scaler.fit_transform(Xtr)
X_test_sd = scaler.transform(Xte)

# GaussianNB
nb = GaussianNB()
nb.fit(X_train_sd, y_train)

y_pred = nb.predict(X_test_sd)
y_prob = nb.predict_proba(X_test_sd)
...
print(f"\nGaussianNB accuracy using top {top_k} features: {acc:.4f}\n")
